...
exports.main = function(options) {
    ...
    function GetMe(){
        ...
        Request1({
            ...
            onComplete: function (response1) {
                console.log("GET ME RESULT == "+response1.text);
    ...
    function GetClassroomImMember(){
        var Request2 = require('request').Request;
        Request2({
            url: "http://dev.afterclassroom.com/api/classrooms/members",
            headers: {
                Authorization: 'Bearer ' + authenticationToken
            },
            onComplete: function (response1) {
    ...
    function GetLinkInfo(){
        var Request2 = require('request').Request;
        console.log("Prepare activeUrl == "+activeUrl);
        console.log("Prepare activeUrl == "+activeUrl);
        console.log("Prepare activeUrl == "+activeUrl);
        console.log("Prepare activeUrl == "+activeUrl);
        Request2({
            url: "http://dev.afterclassroom.com/api/ticks/attach_link",
            headers: {
                Authorization: 'Bearer ' + authenticationToken
            },
            content: {
            ...
            onComplete: function (response1) {
                console.log("debug");
                console.log("debug value of response 1:: == "+response1);
                var jsonResponse = JSON.parse(response1.text);
                console.log("result linkinfo ::: "+response1.text);
                console.log("result linkinfo ::: "+response1.text);
                console.log("result linkinfo ::: "+response1.text);
                console.log("result linkinfo ::: "+response1.text);
    ...
    after_panel.port.on("SubmitLink", function(params_arr) {
        ...
        console.log("BEGIN post link");
        Request({
            url: "http://dev.afterclassroom.com/api/ticks",
            content: {
                title: params_arr[2],
            ...
            //TODO:: hide panel
                console.log("Complete with post link");
                after_panel.hide();
            }
        }).post();
    });
